...
class eBertEncoder(nn.Module):
    def __init__(self, config, lower=True):
        super().__init__()
        self.config = config
        layer_range = config.sep_layers if lower else config.num_hidden_layers - \
                                                    config.sep_layers
        self.layer = nn.ModuleList([BertLayer(config) for _ in range(layer_range)])
        self.gradient_checkpointing = False
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        #last_layer = torch.mean(outputs.hidden_states[-1],axis=1)
        logits = self.classifier(pooled_output)
        #logits = self.classifier(last_layer)
        redefine = False
        '''
        if self.config.problem_type == "overlap":
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                loss = loss_fct(logits, labels)
            elif self.config.problem_type == "overlap":
                logprobs = F.log_softmax(logits, dim=1)
                loss = -1*torch.sum(labels*logprobs,1)
                loss = loss.mean()
        if redefine:
            self.config.problem_type = "overlap"
        if not return_dict:
